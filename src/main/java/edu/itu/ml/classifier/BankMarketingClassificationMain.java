 ...
 * 
 * Learn when people accept or reject an offer from the bank via telephone based
 * on income, age, education and more.
 */
public class BankMarketingClassificationMain {
	...
	public static void main(String[] args) throws Exception {
		List<TelephoneCall> calls = Lists.newArrayList(new TelephoneCallParser(
				"bank-full.csv"));
		...
		for (int run = 0; run < 20; run++) {
			...
			OnlineLogisticRegression lr = new OnlineLogisticRegression(
					NUM_CATEGORIES, TelephoneCall.FEATURES, new L1())
