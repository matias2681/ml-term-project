...
/**
 * A sentimental dictionary where is loaded the file that represent
 * SentiWordNet. Each value is compose by 3 dimensions: positive, negative and
 * objective.
 * 
 */
public class SentimentDictionary {
	...
	private final static Logger LOG = Logger
			.getLogger(SentimentDictionary.class.getName());
	private Map<String, SentimentValue> dic_Senti;
	private Map<String, String> dic_NlpToSenti; // tag pos mapping from NLP
												// format to SentiNetWord format
	...
	public SentimentDictionary() {
		...
		try {
			URL urlPath = this.getClass().getClassLoader()
					.getResource(Constants.FILE_TO_SENTI_WORD_NET);
			...
			while ((line = csv.readLine()) != null) {
				...
				if (!line.trim().startsWith("#")) {
					...
					SentimentValue sentiValue = new SentimentValue(
					...
					for (String synTermSplit : synTermsSplit) {
						...
						tempDictionary.get(synTerm)
								.put(synTermRank, sentiValue);
						// synsetScore);
					}
				}
			}
			...
			for (Map.Entry<String, HashMap<Integer, SentimentValue>> entry : tempDictionary
				...
				for (Map.Entry<Integer, SentimentValue> setScore : synSetScoreMap
						.entrySet()) {
					score.add((SentimentValue) setScore.getValue().div(
			...
			Log.info("Loading the Sentiment Dictionary took: "
	...
	/**
	 * Mapping between OpenNLP tags and SentiWordNet tags.
	 * OpenNLP:
	 * (http://paula.petcu.tm.ro/init/default/post/opennlp-part-of-speech-tags)
	 * 
	 * SentiWordNet
	 * (http://stackoverflow.com/questions/10223314/using-sentiwordnet-3-0) n
	 * NOUN v VERB a ADJECTIVE s ADJECTIVE SATELLITE (not found in data set) r
	 * ADVERB
	 */
	...
	private void create_openNLP_to_SentiWordNet_map() {
		...
		// this.dic_NlpToSenti.put("IN", ""); // Preposition or subordinating
		// conjunction
		...
		this.dic_NlpToSenti.put("VBG", "v"); // Verb, gerund or present
												// participle
		this.dic_NlpToSenti.put("VBN", "v"); // Verb, past participle
		this.dic_NlpToSenti.put("VBP", "v"); // Verb, non-3rd person singular
												// present
		this.dic_NlpToSenti.put("VBZ", "v"); // Verb, 3rd person singular
												// present
	...
	/**
	 * @param word the word
	 * @param pos category of the word
	 * @return the {@link SentimentValue} associated to the word.
	 */
